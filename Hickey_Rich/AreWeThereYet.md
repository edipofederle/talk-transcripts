...
Entãoo herói da palestra de hoje é Alfred North Whitehead. Certo? Ele é um famoso cara que jnnto com Russell escreveu Principia Mathematica. Após isso, ele também se tornou um filoso e escreveu grandes coisas e eu apenas vou coloca-los aqui porque eles são ótimos. Então a primeira coisa é a simplicidade desconfiada. Eu não quero flar sobre a complexidade dos problema que estamos tentando resolver. Todos sabemos que a complexidade está aumentando, grande problemas, mais dados, mais flexibilidade. Expectativa das pessoas para software so tem a crescer. 
...
A complexidade que eu quero falar hoje é a complexidade acidental. A compelxidade que surge da maneira como nossas ferramentas funcionam, a partir das ideias que personificam nossas ferramentas, a partir da maneira como nossas ferramentas não funcionan, a partir da maneira que nossas abordagens não funcionam. Todas essas coisas vem a se tornar probelmas que nós temos que resolver e nós temos uma certa quantidade de horas no dia para resolve-lós. Outros problema que você está resolvendo, os problema da aplicação do domínio ou os problemas que você tem na sua frente por escolher uma linguagem em particular ou ferramenta ou estrátegia de desenvolvimento. Isso é complexidade acidental. Ela aparece no processo. E não é parte do problema que você está tentando resolver.
...
E é pior, eu acho. Quero dizer, todo mundo sabe quando alguma coisa é complexa e você olha para isso, e diz, "Arrr!!! Complexo!" E todo mundo diz, "Okay, Bem, eu vejo isso. Isso é assustado. Eu sei que é uma zona perigosa. Eu sei que terei que ser cuidadoso com isso." O pior tipo de complexidade acidental é aquela disfarçada de simplicidade. "Olhe como isso é fácil!. Não existe ponto e vírgula." Eu não quero bater naqueles ponto e vírgulas. É apenas um jeito fácil de falar, "Olhe, em algum aspecto superficial da linguagem que eu estou usando, isso parece fácil, parece familiar." mas existe complexidade acidental escondida ? 
...
Então, este é um exemplo. Novamente, eu não quero falar contra C++, mas eu passei mais de uma década fazendo isso. Então, não é tão difícil. Quero dizer, se você for para templates e metaprogramming, as coisas podem ficar difíceis. Mas o básico é bastante simples, certo? Você pode escrever uma função que retorna um ponteiro. O que há de errado com isso? Bastante simples. Existem criar e deletar ponteiros e você pode passar ele e você pode desferenciar-los depois. Existe apenas umas cinco coisas que você precisa saber. Você pode aprender isso em uma manhã. Então é bastante simples. Por exemplo, a mesma sintaxe é usada para referenciar coisas em um "heap" e coisas que não estão em um "heap"; aqueles ponteiros. Mas isso fica pior, certo? E o real problema com as assinaturas das funções é o que vocè faz com o resultado que você recebe de uma chamada? Será que é seu? Agora a responsabilidade é sua? Você precisa deletar isso mais tarde? É algo que pode ser deletado? Você pode passa-lo para outra pessoa? Isso é permitido? Poderia você salvar isso?
...
Então o problema é que não existe um padrão automático de gerenciamento de memória. Certo? Não existe um coletor de lixo (GG). E isso era, e ainda é. para as pessoas que usam essa linguagem, um grande problema de complexidade acidental. Certo?  Porque gerenciar a meméoria está em você. Você não vê isso. Não existe um sinal no cabeçalho do seu código fonte, "Não se esqueça; gerenciar a memória está em suas maões". Certo? Isso é complexidade acidental. Você tem que saber isso. Não está no código fonte.
...
Este é um grande problema. Eu penso que a falta de um coletor de lixo realmente impede C++ em um de seus objetivos de projeto, que é ser uma linguagem biblioteca. Todo o projeto original e toda vez que você ouviu Stroustrup falar sobre isso, é C++ se torna uma linguagem biblioteca. Mas ela acabou somente sendo uma linguagem de biblioteca paroquial. Toda loja tem um biblioteca, mas não havia um biblioteca, e ainda não um monte de biblioteca que irão entre os lugares por conta deste problema. E nós sabemos que Java, tem um coletor de lixo, e tem uma enorme biblioteca de infrastrutura. Então eu penso que as pessoas mudaram de C++ para Java fizem em pequena parte pelo fato de não ter mais que lidar com essa complexidade acidental. Eu não quero gerenciar manualmente a memória. Isso não faz parte do problema que estou tentando resolve. É apenas outro problema em meu prato todo dia quando eu vou para o trabalho e eu não quero fazer isso mais.
